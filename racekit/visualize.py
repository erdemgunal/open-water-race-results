from racekit.normalize import age_band, band_sort_key
...
def fmt_pace(sec_per_100m: float) -> str:
    m, s = divmod(int(round(float(sec_per_100m))), 60)
    return f"{m}:{s:02d}"

def _fmt_time_ticks(value: float, _pos=None) -> str:
    return fmt_time(value)

def _fmt_pace_ticks(value: float, _pos=None) -> str:
    return fmt_pace(value)

def load_results(cfg) -> pd.DataFrame:
...
def resolve_user(df: pd.DataFrame, cfg, bib: int | None,
                 time_override: float | None) -> dict:
...
def gaussian_kde_pdf(x_grid: np.ndarray, sample: np.ndarray,
...
def view1_distribution(df: pd.DataFrame, cfg, user: dict, out_dir: Path,
...
def _draw_violin(ax, data, pos: float, color: str, width: float = 0.30):
...
def view2_age_gender(df: pd.DataFrame, cfg, out_dir: Path, show: bool = False) -> Path:
    d = df.copy()
...
def view3_ecdf(df: pd.DataFrame, cfg, user: dict, out_dir: Path, show: bool = False) -> Path:
...
def view4_nations(df: pd.DataFrame, cfg, out_dir: Path, min_entries: int = 5,
                  max_nations: int = 15, show: bool = False) -> Path:
    d = df.copy()
...
def view5_gender_kde(df: pd.DataFrame, cfg, user: dict, out_dir: Path,
                     show: bool = False) -> Path:
...
def run(cfg, bib: int | None = None, time_override: float | None = None,
        min_entries: int = 5, max_nations: int = 15, show: bool = False) -> list[Path]:
    df = load_results(cfg)
